...
export const getLessons = async (_req, res) => {
    try {
        ...
        res.status(200).json(lessons.map((lesson) => ({
            id: lesson.id,
            title: lesson.title,
            chapterId: lesson.chapterId,
            chapterTitle: lesson.chapter?.title || "Unassigned",
            media: lesson.media && !lesson.media.startsWith("http")
                ? `${API_URL}${lesson.media.startsWith("/") ? "" : "/"}${lesson.media}`
                : lesson.media || null,
            questions: lesson.questions.map((q) => ({
                id: q.id,
                question: q.question,
                questionImage: q.questionImage && !q.questionImage.startsWith("http")
                    ? `${API_URL}${q.questionImage.startsWith("/") ? "" : "/"}${q.questionImage}`
...
export const updateLesson = async (req, res) => {
    try {
        ...
        if (media)
            updateData.media = media;
        if (status)
            updateData.status = status; // ✅ ADD THIS LINE
        ...
        if (pages) {
            ...
            const idsFromClient = parsedPages.map((p) => p.id).filter(Boolean);
            const deletedPages = existingPages.filter((p) => !idsFromClient.includes(p.id));
        ...
        const pagesWithMediaURL = updated?.pages.map((p) => ({
            content: p.content,
            media: p.media
                ? p.media.startsWith("/uploads/")
                    ? `${API_URL}${p.media}`
...
export const createLesson = async (req, res) => {
    try {
        ...
        console.error("❌ Error creating lesson:", error instanceof Error ? error.message : error);
        if (error instanceof Error) {
 ...
 */
export const deleteContentImage = (async (req, res) => {
    try {
 ...
 * ✅ DELETE CHOICE IMAGE
 */
export const deleteChoiceImage = (async (req, res) => {
    try {
